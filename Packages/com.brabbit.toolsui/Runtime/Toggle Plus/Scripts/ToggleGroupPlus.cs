...
namespace UnityEngine.UI {
    ...
    public class ToggleGroupPlus : UIBehaviour {
        ...
        public bool allowInteractableOff { get { return m_AllowInteractableOff; } set { m_AllowInteractableOff = value; } }

        protected List<TogglePlus> m_Toggles = new List<TogglePlus>();
        ...
        public void NotifyToggleOn(TogglePlus toggle, bool sendCallback = true) {
            ValidateToggleIsInGroup(toggle);
            // disable all toggles in the group
            for (var i = 0; i < m_Toggles.Count; i++) {

                if (m_Toggles[i] == toggle) {
                    if(allowInteractableOff)
                        toggle.interactable = !toggle.isOn;

                    onToggleIsOn.Invoke(toggle);
                    continue;
                }
        ...
        public void EnsureValidState() {
            if (!allowSwitchOff && !AnyTogglesOn() && m_Toggles.Count != 0) {
                ...
                m_Toggles[0].interactable = false;

                NotifyToggleOn(m_Toggles[0]);
            }

            IEnumerable<TogglePlus> activeToggles = ActiveToggles();

            if (activeToggles.Count() > 1) {
                TogglePlus firstActive = GetFirstActiveToggle();

                foreach (TogglePlus toggle in activeToggles) {
        ...
        /// </remarks>
        public TogglePlus GetFirstActiveToggle() {
            IEnumerable<TogglePlus> activeToggles = ActiveToggles();
